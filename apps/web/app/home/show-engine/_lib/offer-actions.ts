...
export async function parseOfferSheet(input: {
  ...
  const res = await callClaude({
    ...
    maxTokens: 1200,
    system:
      'You extract structured fields from a live-music show offer sheet. Return ONLY valid JSON (no markdown) with any fields you can find: {"show_name":string,"show_date":"YYYY-MM-DD","venue_capacity":number,"avg_ticket_price":number,"offer_structure":"straight_guarantee|backend|hybrid|bonus_escalator|vs_deal|pure_door","guarantee":number,"promoter_profit":number,"backend_promoter_share":number,"fixed_show_expenses":number,"bonus_tiers":[{"from_attendance":number,"to_attendance":number,"bonus_paid":number}],"conservative_attendance":number,"target_attendance":number,"sellout_attendance":number}. Structure cues: a headline like "versus ... whichever is greater" (greater of guarantee or the door split) => vs_deal; "plus ... of NBOR after expenses" => backend; a Split Point line equal to expenses + promoter profit with NO guarantee => pure_door. Extract the sheet\'s "Promoter Profit" line into promoter_profit. Omit fields you cannot determine. Do not guess wildly — only include values clearly supported by the sheet.',
    messages: [
      {
        role: 'user',
        content: [
